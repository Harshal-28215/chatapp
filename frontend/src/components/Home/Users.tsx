...
const Users = memo(({ data, onlineUsers, setIsOpen }: { data: SidebarUsers, onlineUsers: string[] | undefined, setIsOpen: (isOpen: boolean) => void }) => {
    ...
    return (
        <div className='flex gap-2.5' key={data._id}>
            ...
            <Link to={`/chat/${data._id}`} className='w-[calc(100%-60px)]' onClick={() => window.innerWidth < 640 ? setIsOpen(false) : null}>
                <h3>{data.name}</h3>
